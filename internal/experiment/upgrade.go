...
func Upgrade(ctx context.Context, playerPath, callbacksPath string) error {
	...
	if err := RunCmdSilent(ctx, playerPath, "npm", "install", "--silent", "@empirica/core@latest"); err != nil {
		return errors.Wrap(err, "upgrade client")
	}

	if err := RunCmdSilent(ctx, callbacksPath, "npm", "install", "--silent", "@empirica/core@latest"); err != nil {
