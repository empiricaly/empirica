...
Empirica.onPlayerConnected(function ({ player }) {
  console.log("player connecteds", player.id);
...
Empirica.onNewBatch(function ({ batch }) {
  ...
  if (!isObject(conf) || !conf["kind"]) {
    console.warn("callbacks: batch config is invalid", conf);
    return;
  }
...
Empirica.onChange("player", "introDone", function ({ isNew, player }) {
  ...
  const playerCount = game.get("treatment")["playerCount"];
...
export { Empirica as DefaultCallbacks };
